...
; КРИТИЧЕСКОЕ ЗАМЕЧАНИЕ - программа начинается с метки start в сегменте RM_seg; затем, проделывая нессколько нехитрых (лол, блять) манипуляций,
;мы переходим в защищенный режим на PM_Entry сегмента PM_seg
; КРИТИЧЕСКОЕ ЗАМЕЧАНИЕ №2 - автор кода настоятельно НЕ РЕКОМЕНДУЕТ трогать работающую программу, вообще, никак, вплоть до того что нельзя даже
;переставить макрос А перед макросом Б, а сегмент защищенного режима - после сегмента реального, как и происходит по хронологии
;просто потому что именно в данном порядке оно РАБОТАЕТ, а не выдаёт абстрактные не привязанные к реальности значения
; КРИТИЧЕСКОЕ ЗАМЕЧАНИЕ №3 - важен не только код перевода в [s]ад[/s] защищенный режим и обратно, но и обработчики прерываний таймера и клавиатуры
;соответственно - new_int08 (таймер) и new_int09 (клавиатура); переход обратно в реальный режим происходит чуть ниже метки work (после вываливания из цикла)
...
; Happy End, компания interassembly transitions благодарит вас за интерес в ассемблерной магии и использовании бауманских методичек
;а теперь не поленись, перечитай с самого начала, будь мужиком блеать, всё равно ведь нихрена не понял С: