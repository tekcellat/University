...
# запускаем 3 раза, 1й ничего не меняем, далее по комментам чекай
# первый слой (вход) на 15, второй на 300 и третий (выход) 10 нейронов
...
def test_network(network):
    ...
    print(
        'Test accuracy: {:.2f}%'.format(
            float(total_classifications - misc) / total_classifications * 100 # на второй запуск добавляем - 50
            # float(total_classifications - misc) / total_classifications * 100 - 50
        )
    )
